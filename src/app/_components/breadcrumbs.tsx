...
export default function Breadcrumbs() {
  ...
  const { data: prompt } = api.prompt.get.useQuery(
    { promptId },
    { enabled: !!promptId },
  );
  ...
  useEffect(() => {
    ...
    if (parts[0] == "t" && parts[1]) {
      setTeamId(parts[1]);
      if (team)
        newPieces.push({ name: `Team: ${team.name}`, url: `/t/${parts[1]}` });
    }
    if (parts[2] == "p" && parts[3]) {
      // const [project] = api.project.get.useSuspenseQuery({ id: parts[3] });
      setProjectId(parts[3]);
      if (project)
        newPieces.push({
          name: `Project: ${project.name}`,
          url: `/t/${parts[1]}/p/${parts[3]}`,
        });

      if (parts[4]) {
        //   const [prompt] = api.prompt.get.useSuspenseQuery({ promptId: parts[4] });
        setPromptId(parts[4]);
        if (prompt)
          newPieces.push({
            name: `Prompt: ${prompt.name}`,
            url: `/t/${parts[1]}/p/${parts[3]}/${parts[4]}`,
          });
      }
  ...
  return (
    <nav aria-label="Breadcrumb" className="flex">
      <ol
        role="list"
        className="mx-auto flex w-full max-w-screen-xl space-x-4 bg-[#2e026d] px-4 pt-2 sm:px-6 lg:px-8"
      >
        <li className="flex">
